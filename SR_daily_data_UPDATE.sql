...
CREATE DEFINER=`smartform_user`@`localhost` PROCEDURE `SR_daily_data_UPDATE`()
BEGIN
    ...
    CREATE TABLE SR_daily_data AS
        ...
        FROM
            coolwed1_WP9PN.daily_races_beta dracb
            JOIN coolwed1_WP9PN.daily_runners_beta drunb ON (drunb.race_id = dracb.race_id)
            LEFT JOIN coolwed1_WP9PN.historic_runners_beta hrunb ON (hrunb.race_id = dracb.race_id AND hrunb.runner_id = drunb.runner_id)
        ;
END$$
