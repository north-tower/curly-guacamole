DELIMITER $$
CREATE PROCEDURE `speed_analysis_UPDATE`()
BEGIN
	...
	CREATE TABLE speed_analysis as
		...
		FROM
			...
			FROM coolwed1_wp364.my_daily_details_tb mddt 
            ...
            JOIN historic_races_beta hracb ON (hrunb.race_id = hracb.race_id)
				LEFT JOIN coolwed1_wp364.sr_results sr ON (sr.runner_id = mddt.runner_id AND sr.race_id = hrunb.race_id)
