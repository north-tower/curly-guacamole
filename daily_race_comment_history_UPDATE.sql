...
CREATE DEFINER=`smartform_user`@`localhost` PROCEDURE `daily_race_comment_history_UPDATE`()
BEGIN
    DROP TABLE IF EXISTS daily_race_comment_history;
    CREATE TABLE daily_race_comment_history AS 
    SELECT DISTINCT `hrunb`.`runner_id` AS `runner_id`,
    ...
    FROM
        coolwed1_WP9PN.daily_comment_history dch
            LEFT JOIN
        coolwed1_WP9PN.historic_runners_beta hrunb ON (dch.race_id = hrunb.race_id)
            LEFT JOIN
        coolwed1_WP9PN.historic_races_beta hracb ON (dch.race_id = hracb.race_id)
            LEFT JOIN
        coolwed1_WP9PN.sr_results `sr` ON ((`dch`.`race_id` = `sr`.`race_id`)
